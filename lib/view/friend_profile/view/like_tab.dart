...
class LikeTab extends StatefulWidget {
  ...
  const LikeTab({this.userID});

  @override
  _LikeTabState createState() => _LikeTabState();
}
...
class _LikeTabState extends State<LikeTab> with AutomaticKeepAliveClientMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return WhenRebuilderOr<FriendProfileProvider>(
        ...
        initState: (_, model) => model.setState((s) => s.getUserLikePost(widget.userID)),
        builder: (context, model) => FRefresh(
              ...
              child: Container(
                  ...
                  width: Get.width,
                  child: model.state.postLikeCount == 0
                      ? Center(child: Text("No like"))
                      : ListView.builder(
                          physics: NeverScrollableScrollPhysics(),
                          itemCount: model.state.postLikeCount,
