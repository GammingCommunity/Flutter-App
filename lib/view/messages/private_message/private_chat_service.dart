...
class PrivateChatService {
  ...
  static Future<List<User>> getFriendName(List member) async {
    ...
    return [
      User(id: int.parse(userInfo['userID']), nickname: userInfo['userName'],profileUrl: userInfo['profileUrl']),
      User(id: friendName[0].id, nickname: friendName[0].nickname,profileUrl: friendName[0].profileUrl)
    ];
  }

  static void chatText(Socket socket, String content) async {
    var userInfo = await getUserInfo();
    var userID = userInfo['userID'];
    socket.emit('chat-private', [
      [
        {'chatID': ""},
        {"sender": userID, "text": content}
      ]
    ]);
  }
